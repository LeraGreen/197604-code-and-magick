...
const Statistics = function (ctx, names, times) {
  this.names = names;
  this.ctx = ctx;
  ...
  this.renderColumns = function () {
    ...
    this.names.forEach(function (name, index) {
      const x = this.cloudX + this.columnOffset + ((this.columnWidth + this.columnOffset) * index);
      const y = Math.round((-this.columnMaxHeight * this.times[index]) / max);
...
window.renderStatistics = function (ctx, names, times) {
  const statistics = new Statistics(ctx, names, times);
